...
function CustomNumberFormat(locale?: string | string[], options?: Intl.NumberFormatOptions): Intl.NumberFormat {
  ...
  if (
    normalizedOptions.roundingMode &&
    normalizedOptions.maximumFractionDigits !== undefined
  ) {
    ...
    return {
      format: (value: number | bigint | string) => {
        ...
        const multiplier = 10 ** restOptions.maximumFractionDigits!
          ...
          case "halfEven": {
            ...
            if (Math.abs(fraction) === 0.5) {
              roundedValue =
                (truncated % 2 === 0
                  ? truncated
                  : truncated + Math.sign(fraction)) / multiplier
            } else {
