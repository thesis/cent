 ...
 */
export function nowUNIXTime(precision: 'seconds' | 'milliseconds' = 'milliseconds'): UNIXTime {
  const now = Date.now()
 ...
 */
export function dateToUNIXTime(date: Date, precision: 'seconds' | 'milliseconds' = 'milliseconds'): UNIXTime {
  const timestamp = precision === 'seconds' ? Math.floor(date.getTime() / 1000) : date.getTime()
