...
export const FractionStringSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Denominator cannot be zero and both numerator and denominator must be valid integers",
    },
  )
...
export function parseFraction(fractionStr: string): Ratio {
  ...
  if (!result.success) {
    throw new Error(
      `Invalid fraction format: "${fractionStr}". ${result.error.message}`,
    )
  }
 ...
 */
export function getRationalStringType(
  rationalStr: string,
): "fraction" | "decimal" {
